...
import {
  ...
  transformPolygonTransactionToTransaction,
...
export class PolygonAdapter extends NetworkAdapter {
  ...
  async getBlock(blockNumber: BlockNumberOrTag): Promise<DataWithMetadata<Block>> {
    ...
    const block = transformPolygonBlockToBlock(blockData);
  ...
  async getBlockWithTransactions(
    ...
    if (Array.isArray(blockData.transactions)) {
      for (const tx of blockData.transactions) {
        if (typeof tx !== "string") {
          transactionDetails.push(transformPolygonTransactionToTransaction(tx));
        }
      }
    }
  ...
  async getTransaction(txHash: string): Promise<DataWithMetadata<Transaction>> {
    ...
    const transaction = transformPolygonTransactionToTransaction(txData, receiptData);
