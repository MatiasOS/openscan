...
export class RPCClient {
  ...
  private async callWithFallback<T = any>(
    ...
    if (urlIndex >= this.rpcUrls.length) {
      ...
      const errorMessages = errors.map((e, i) => `  [${i + 1}] ${this.rpcUrls[i]}: ${e.message}`).join('\n');
  ...
  async call<T = any>(method: string, params: any[] = []): Promise<T> {
    ...
    const orderedIndices = [
      ...
      ...Array.from({ length: this.rpcUrls.length }, (_, i) => i).filter(i => i !== this.currentUrlIndex)
    ];
    ...
    for (const urlIndex of orderedIndices) {
      try {
        ...
        if (error instanceof Error && error.message.startsWith('All RPC endpoints failed')) {
          throw error;
        }
  ...
  async batchCall<T = any>(calls: Array<{ method: string; params: any[] }>): Promise<T[]> {
    ...
    // All URLs failed
    const errorMessages = allErrors.map((e, i) => `  [${i + 1}] ${this.rpcUrls[i] || 'unknown'}: ${e.message}`).join('\n');
