...
const ANKR_BASE = "https://rpc.ankr.com";

export async function evmAnkrHandler(c: Context<{ Bindings: Env }>) {
  ...
  const url = `${ANKR_BASE}/${network.ankr}/${c.env.ANKR_API_KEY}`;
...
export async function btcAnkrHandler(c: Context<{ Bindings: Env }>) {
  const body = c.get("validatedBody" as never) as unknown as BtcRpcRequestBody;
  const url = `${ANKR_BASE}/btc/${c.env.ANKR_API_KEY}`;
